...
def main():
    ...
    dft_nmr = df[9].values  # 6th column (DFT values)
    ml_nmr = df[19].values   # 16th column (ML values)
    ...
    create_joint_plot(
        dft_nmr, ml_nmr,
        r'$^{129}$Xe $\sigma_{xz}^{\text{DFT}}$ / ppm', r'$^{129}$Xe $\sigma_{xz}^{\text{ML}}$ / ppm',
        'comparison_sigma_xz.svg',
        scale_factor=1.0
    )
    
    print("\nAnalysis complete. Plot has been saved as 'comparison_sigma_xz.svg' and 'comparison_sigma_xz.png'.")
