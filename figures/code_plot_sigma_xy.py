...
def main():
    ...
    dft_nmr = df[8].values  # 6th column (DFT values)
    ml_nmr = df[18].values   # 16th column (ML values)
    ...
    create_joint_plot(
        dft_nmr, ml_nmr,
        r'$^{129}$Xe $\sigma_{xy}^{\text{DFT}}$ / ppm', r'$^{129}$Xe $\sigma_{xy}^{\text{ML}}$ / ppm',
        'comparison_sigma_xy.svg',
        scale_factor=1.0
    )
    
    print("\nAnalysis complete. Plot has been saved as 'comparison_sigma_xy.svg' and 'comparison_sigma_xy.png'.")
