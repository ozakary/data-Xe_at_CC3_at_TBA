...
def create_joint_plot(x_data, y_data, xlabel, ylabel, filename, scale_factor=1.0):
    ...
    stats_text = r'R$^{2}$' + f'={r_value**2:.5f}\nRMSE = {rmse:.1f}\nMAE = {mae:.1f}'
...
def main():
    ...
    dft_nmr = df[11].values  # 6th column (DFT values)
    ml_nmr = df[21].values   # 16th column (ML values)
    ...
    create_joint_plot(
        dft_nmr, ml_nmr,
        r'$^{129}$Xe $\sigma_{yy}^{\text{DFT}}$ / ppm', r'$^{129}$Xe $\sigma_{yy}^{\text{ML}}$ / ppm',
        'comparison_sigma_yy.svg',
        scale_factor=1.0
    )
    
    print("\nAnalysis complete. Plot has been saved as 'comparison_sigma_yy.svg' and 'comparison_sigma_yy.png'.")
