...
def main():
    ...
    dft_nmr = df[7].values  # 6th column (DFT values)
    ml_nmr = df[17].values   # 16th column (ML values)
    ...
    create_joint_plot(
        dft_nmr, ml_nmr,
        r'$^{129}$Xe $\sigma_{xx}^{\text{DFT}}$ / ppm', r'$^{129}$Xe $\sigma_{xx}^{\text{ML}}$ / ppm',
        'comparison_sigma_xx.svg',
        scale_factor=1.0
    )
    
    print("\nAnalysis complete. Plot has been saved as 'comparison_sigma_xx.svg' and 'comparison_sigma_xx.png'.")
